...
export const GeneralInfoForm = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (currentStep === 1) {
        const response = await axios.post(BaseUrl + `/labAuth/labAuthCreate`, {
        ...
        const response = await axios.post(BaseUrl + `/labAuth/labAuthProfile`, {
  ...
  const handleOTPVerification = async () => {
    ...
    try {
      const response = await axios.post(BaseUrl + `/labAuth/labVerifyOTP`, {
        fullName: formData.fullName,
        emailId: formData.emailId,
